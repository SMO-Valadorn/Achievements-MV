...
*
* Position
* When writing formulas for the X and Y coordinates, one can use PopUp.width and
...
*
* You can use sort() to order the achievements, check the default value on
...
*    - Achievements are now refreshed after transferring;
...
SMO.AM.setupAchievements = function () {
  var d, extras;
  var changed = false;
  this.achievs = SMO.AM.Achievements().achievs;
  this.trophies = SMO.AM.Achievements().trophies;
...
Game_System.prototype.setupAchievs = function () {
  if (SMO.AM.Achievements().achievs != undefined &&
    this.achievPopUp != undefined) {
    //if (!SMO.AM.isGlobalRange) {
    SMO.AM.setupAchievements();
    //}

    if (SMO.AM.Sort.enabled) {
      if (!SMO.AM.Sort.options[SMO.AM.Achievements().achievs.sortType]) {
...
Scene_Base.prototype.updateAchievements = function () {
  ...
  if (this instanceof Scene_Title || this instanceof Scene_TitleMap && !SMO.AM.isGlobalRange) return;
