...
* @dir img/achievements
* @require 1
* @desc Choose an image as background for all the locked (non secret) the achievements.
* @default
*
...
* @dir img/achievements
* @require 1
* @desc Choose an image as background for all the secret the achievements.
* @default
*
* @param Auto Refresh
* @type boolean
...
* will also match the achievement id. For organization purposes you can add
...
*
* @param Trophies
* @desc This name will appear above the trophies.
* @default TROPHIES
*
* @param None
...
Achievement_Data.prototype.unlock = function () {
  ...
  if (SMO.AM.Achievements().achievPopUp.queue.length === 1) {
...
Achievement_Data.prototype.lock = function () {
  ...
  if (index > -1) {
    ...
    if (PopUp) {
      ...
      if (popUpIndex != 0) {
        SMO.AM.Achievements().achievPopUp.queue.delete(this.id);
...
if (PluginManager.parameters('SMO_Achievements') != undefined
  ...
  //Default Icons
  SMO.AM.Icons = {
    locked: PluginManager._parameters['smo_achievements']['Locked Icon'] ? Number(PluginManager._parameters['smo_achievements']['Locked Icon']) : -1,
    unlocked: PluginManager._parameters['smo_achievements']['Unlocked Icon'] ? Number(PluginManager._parameters['smo_achievements']['Unlocked Icon']) : -1,
    secret: PluginManager._parameters['smo_achievements']['Secret Icon'] ? Number(PluginManager._parameters['smo_achievements']['Secret Icon']) : -1,
    gold: PluginManager._parameters['smo_achievements']['Gold Icon'] ? Number(PluginManager._parameters['smo_achievements']['Gold Icon']) : -1,
    recentUnlock: PluginManager._parameters['smo_achievements']['Recent Unlock'] ? Number(PluginManager._parameters['smo_achievements']['Recent Unlock']) : -1
  };

  //Menu Command
  SMO.AM.MenuCommand = {
    active: PluginManager._parameters['smo_achievements']['Menu Command'] === 'true' ? true : false,
    switchId: Number(PluginManager._parameters['smo_achievements']['Show Command Switch']),
    position: Number(PluginManager._parameters['smo_achievements']['Command Position'])
  };
  ...
  //Sort Options
  SMO.AM.Sort = {
    options: PluginManager._parameters['smo_achievements']['Sort Options'] ? JSON.parse(PluginManager._parameters['smo_achievements']['Sort Options']) : [],
    cellHeight: Number(PluginManager._parameters['smo_achievements']['Cell Height']),
    x: Number(PluginManager._parameters['smo_achievements']['Sort X']),
    y: Number(PluginManager._parameters['smo_achievements']['Sort Y']),
    width: Number(PluginManager._parameters['smo_achievements']['Sort Width'])
  }
...
SMO.AM.getCategories = function () {
  if (PluginManager.parameters('SMO_Achievements') != undefined
    ...
    cat.forEach(function (c) {
      ...
      SMO.AM.categoryNames.push(data['Category Name']);
    });
    if (SMO.AM.Categories.length > 99) {
      SMO.AM.Categories.splice(99, SMO.AM.Categories.length - 99);
      SMO.AM.categoryNames.splice(99, SMO.AM.categoryNames.length - 99);
      console.warn('The categories are hard capped at 99 members. Extra categories removed.');
    }
    if (SMO.AM.categoryNames.contains('none')) {
      SMO.AM.categoryNames.deleteAll('none');
...
SMO.AM.getTexts = function () {
  if (PluginManager.parameters('SMO_Achievements') != undefined
    && isEmpty(PluginManager._parameters['smo_achievements']) == false) {
    var texts = JSON.parse(PluginManager._parameters['smo_achievements']['Texts and Colors']);
    SMO.AM.Texts = {
...
if (PluginManager.parameters('SMO_Achievements') != undefined
  && isEmpty(PluginManager._parameters['smo_achievements']) == false) {
  SMO.AM.isGlobalRange = String(PluginManager._parameters['smo_achievements']['Range']) === 'Global' ? true : false;
  ...
  SMO.AM.isGlobalRewards = SMO.AM.isGlobalRange && String(PluginManager._parameters['smo_achievements']['Global Rewards']) === 'true' ? true : false;
  SMO.AM.background = String(PluginManager._parameters['smo_achievements']['Menu Background']);
  SMO.AM.autoRefresh = PluginManager._parameters['smo_achievements']['Auto Refresh'] === 'true' ? true : false;
  SMO.AM.updateInterval = Number(PluginManager._parameters['smo_achievements']['Update Interval']);
  SMO.AM.onUnlockScript = PluginManager._parameters['smo_achievements']['On Unlock'] ? JSON.parse(PluginManager._parameters['smo_achievements']['On Unlock']) : '';
  SMO.AM.hideTotally = PluginManager._parameters['smo_achievements']['Hide Totally'] === 'true' ? true : false;
  SMO.AM.lockedBackground = PluginManager._parameters['smo_achievements']['Locked Achiev Background'];
  SMO.AM.secretBackground = PluginManager._parameters['smo_achievements']['Secret Achiev Background'];

  SMO.AM.TrophiesConfigs = {
    enabled: PluginManager._parameters['smo_achievements']['Use Trophies'] === 'true' ? true : false,
    lines: Number(PluginManager._parameters['smo_achievements']['Trophies Lines']),
    cols: Number(PluginManager._parameters['smo_achievements']['Trophies Columns']),
    selectStyle: PluginManager._parameters['smo_achievements']['Trophy Selector'] === 'Grow' ? 'grow' : 'cursor'
  };

  SMO.AM.TitleCommand = {
    active: PluginManager._parameters['smo_achievements']['Title Command'] === 'true' ? true : false,
    name: String(PluginManager._parameters['smo_achievements']['Title Command Name']),
    position: Number(PluginManager._parameters['smo_achievements']['Title Command Position']),
  }

  //Pop Up
  SMO.AM.PopUp = {
    state: PluginManager._parameters['smo_achievements']['Pop Up Window'] === 'true' ? true : false,
    button: PluginManager._parameters['smo_achievements']['Pop Up Button'] === 'true' ? true : false,
    text: PluginManager._parameters['smo_achievements']['Pop Up Text'] ? JSON.parse(PluginManager._parameters['smo_achievements']['Pop Up Text']) : '',
    x: String(PluginManager._parameters['smo_achievements']['Pop Up X']),
    y: String(PluginManager._parameters['smo_achievements']['Pop Up Y']),
    width: String(PluginManager._parameters['smo_achievements']['Pop Up Width']),
    height: String(PluginManager._parameters['smo_achievements']['Pop Up Height']),
    borderColor: PluginManager._parameters['smo_achievements']['Pop Up Borders'] || 'rgba(0,0,0,0)',
    preselect: 0,
...
SMO.AM.setupAchievements = function () {
  var d, extras;
  var changed = false;
  var all = SMO.AM.Achievements().achievs.locked.length + SMO.AM.Achievements().achievs.unlocked.length;
  if (SMO.AM.Data.length != all) {
    changed = true;
    extras = SMO.AM.Data.length - all;
    if (extras > 0) {//data added
      for (d = all; d < SMO.AM.Data.length; d++) {
        SMO.AM.Achievements().achievs.locked.push(d + 1);
      }
    } else {//data removed
      for (d = all; d > SMO.AM.Data.length; d--) {
        SMO.AM.Achievements().achievs.locked.delete(d);
        SMO.AM.Achievements().achievs.unlocked.delete(d);
        SMO.AM.Achievements().achievs.unlockDate.delete(d - 1);
        SMO.AM.Achievements().achievs.recentUnlock.delete(d);
      }
    }
  }
  all = SMO.AM.Achievements().trophies.locked.length + SMO.AM.Achievements().trophies.unlocked.length;
  if (SMO.AM.Categories.length != all) {
    ...
    if (extras > 0) {//trophies added
      for (d = all; d < SMO.AM.Categories.length; d++) {
        SMO.AM.Achievements().trophies.locked.push(d + 1);
      }
    } else {//trophies removed
      for (d = all; d > SMO.AM.length; d--) {
        SMO.AM.Achievements().trophies.locked.delete(d - 1);
        SMO.AM.Achievements().trophies.unlocked.delete(d - 1);
      }
    }
  }
...
SMO.AM.initializeGlobalAchievements = function () {
  if (typeof SMO.AM.GlobalAchievements == "undefined"
    || isEmpty(SMO.AM.GlobalAchievements)) {
 ...
 */
function merge_options(loadedData, defaultData) {
  var obj3 = {};
  for (var attrname in loadedData) { obj3[attrname] = loadedData[attrname]; }
  for (var attrname in defaultData) {
    if (Object.keys(defaultData[attrname]).length != Object.keys(obj3[attrname]).length) {
      for (var attribute in defaultData[attrname]) {
        if (typeof obj3[attrname][attribute] == 'undefined') {
          obj3[attrname][attribute] = defaultData[attrname][attribute];
        }
      }
    }
  }
...
SMO.AM.showAchievements = function (category) {
  if (category != undefined && SMO.AM.categoryNames.contains(category)) {
...
SMO.AM.refreshAchievements = function () {
  ...
  for (var d = 0; d < SMO.AM.Achievements().achievs.locked.length; d++) {
...
SMO.AM.unlockAchievement = function (id) {
  ...
  if (isNumber) {
    ...
    if (achievement && !achievement.isUnlocked()) {
      ...
      SMO.AM.Achievements().achievPopUp.queue.push(isNumber);
...
SMO.AM.resetAchievementsData = function () {
  ...
  SMO.AM.Achievements().achievPopUp = {
    opacity: 0,
    isFading: false,
    isShowing: false,
    fadeRate: 0,
...
SMO.AM.getRequirementValues = function (requirement, achievement) {
  ...
  var iconIndex;
  var comparison = requirement.comparison;
  var currentValue;
  var finalValue;
  var name;
  var condition;
  ...
  if (this.achievs) {
    ...
    if (SMO.AM.Sort.enabled) {
      if (!SMO.AM.Sort.options[this.achievs.sortType]) {
        SMO.AM.Achievements().achievs.sortType = 0;
...
Game_System.prototype.isTrophyUnlocked = function (trophyId) {
  ...
  if (id) {
    ...
    var trophyIndex = SMO.AM.categoryNames.indexOf(trophyId);
    if (trophyIndex > -1) {
...
Scene_Base.prototype.updateAchievements = function () {
  ...
  if (typeof Scene_TitleMap != "undefined") {
    if (this instanceof Scene_TitleMap) return;
  }
...
Achievement_PopUp.prototype.initialize = function () {
  ...
  this._timerX = 180;
  SMO.AM.Achievements().achievPopUp = SMO.AM.Achievements().achievPopUp || {};
  this.opacity = SMO.AM.Achievements().achievPopUp.opacity || 0;
  this._fadeRate = SMO.AM.Achievements().achievPopUp.fadeRate || 0;
  this._fadeLimit = SMO.AM.Achievements().achievPopUp.fadeLimit || 0;
  this._isFading = SMO.AM.Achievements().achievPopUp.isFading || false;
  this._isShowing = SMO.AM.Achievements().achievPopUp.isShowing || false;
  this._timer = SMO.AM.Achievements().achievPopUp.timer || 0;
  this._queue = SMO.AM.Achievements().achievPopUp.queue || [];
  this._isBusy = this._queue.length > 0;
  this.drawAchievementPopUp();
  if (SMO.AM.Popup && SMO.AM.PopUp.button) {
...
Achievement_PopUp.prototype.onClick = function () {
  if (this.opacity > 0) {
    ...
    if (achievement) {
      ...
      SMO.AM.currentCategory = SMO.AM.categoryNames.length > 0 ? category : 'none';
      ...
      data = Window_Achievements.prototype.sortData(data, SMO.AM.Achievements().achievs.sortType);
  ...
  if (this._queue.length > 0) {
    ...
    if (achievement) {
      ...
      if (achievement.isMyPopUpReady()) {
        ...
        SMO.AM.Achievements().achievPopUp.isShowing = true;
        this.fade(fadeRate, this._timerX);//fade in
      } else {
        //if the background is not ready the achievement is pushed to the end of
        //the queue and the queue will wait 15 frames before continue
        SMO.AM.Achievements().achievPopUp.queue.push(this._queue[0]);
        SMO.AM.Achievements().achievPopUp.queue.splice(0, 1);
...
Achievement_PopUp.prototype.drawTextEx = function (text, x, y) {
  if (this.bitmap) {
    if (text) {
      ...
      //Drawing the texts
      var textState = { index: 0, x: x, y: y, text: "", left: x, height: 0, xOffSet: xOffSet, lineIndex: 0 };
      textState.text = Window_Base.prototype.convertEscapeCharacters(text);
...
Achievement_PopUp.prototype.fade = function (fadeRate, timer, fadeLimit) {
  ...
  timer = timer || 0;
  SMO.AM.Achievements().achievPopUp.fadeLimit = fadeLimit;
  SMO.AM.Achievements().achievPopUp.fadeRate = fadeRate;
  SMO.AM.Achievements().achievPopUp.timer = timer;
  SMO.AM.Achievements().achievPopUp.isFading = true;
...
Achievement_PopUp.prototype.update = function () {
  ...
  if (this._isFading) {
    if (this._fadeRate > 0) {
      ...
      SMO.AM.Achievements().achievPopUp.opacity = this.opacity;
      SMO.AM.Achievements().achievPopUp.isFading = this._isFading;
      ...
      if (this.opacity > this._fadeLimit) {
        ...
        SMO.AM.Achievements().achievPopUp.isShowing = false;
        this._isFading = false;
      }
      SMO.AM.Achievements().achievPopUp.opacity = this.opacity;
      SMO.AM.Achievements().achievPopUp.isFading = this._isFading;
  ...
  if (this._timer && !this._isFading) {
    this._timer--;
    SMO.AM.Achievements().achievPopUp.timer--;
    if (this._timer <= 0) {
      this.hide();
      this._timer = 0;
      SMO.AM.Achievements().achievPopUp.timer = 0;
    }
  }
...
Scene_Achievements.prototype.createTrophiesWindow = function () {
  if (SMO.AM.categoryNames.length > 0) {
...
Scene_Achievements.prototype.isCategory = function () {
  return SMO.AM.categoryNames.length > 0 && SMO.AM.currentCategory === 'none';
}
...
Scene_Achievements.prototype.onAchievementCancel = function () {
  if (SMO.AM.currentCategory === 'none') {
    ...
    var categoryIndex = SMO.AM.categoryNames.indexOf(SMO.AM.currentCategory);
    SMO.AM.currentCategory = 'none';
...
Sort_Option.prototype.initialize = function () {
  ...
  if (SMO.AM.Achievements().achievs.sortType == undefined) {
    this._selected = 0;
  }
  else {
    this._selected = SMO.AM.Achievements().achievs.sortType;
  }
...
document.onpaste = function (event) {
  if (Sprite_InteractiveP._typing) {
    ...
    if (text) {
      ...
      var fittingText = text.substr(0, maxEntries);
